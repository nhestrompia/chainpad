...
APP_NAME="ChainPad"
DIST_DIR="dist"
CASK_PATH="Casks/chainpad.rb"
NOTARY_PROFILE="chainpad-notary"
VERSION=""
REPO=""
IDENTITY=""
...
Options:
  ...
  --notary-profile <profile>       notarytool keychain profile (default: chainpad-notary)
  --dist <path>                    Output directory (default: dist)
  --cask-path <path>               Cask file path (default: Casks/chainpad.rb)
...
for cmd in swift codesign xcrun ditto shasum awk sed; do
...
./scripts/package-macos-app.sh --version "${VERSION}" --dist "${DIST_DIR}"
...
if [[ -f "${PKG_PATH}" ]]; then
  ...
  printf "%s  %s\n" "${PKG_SHA}" "$(basename "${PKG_PATH}")" >> "${CHECKSUMS_PATH}"
fi
...
if [[ "${UPLOAD_RELEASE}" == "true" ]]; then
  ...
  if gh release view "${TAG}" >/dev/null 2>&1; then
    ...
    if [[ -f "${PKG_PATH}" ]]; then
      gh release create "${TAG}" "${ZIP_PATH}" "${PKG_PATH}" "${CHECKSUMS_PATH}" \
        --title "${TAG}" \
        --notes "ChainPad ${TAG}"
    else
